...
def broadcast_tensor_in_dp_group(size, dtype, tensor=None):
    """Broadcast a tensor from the first rank in the data parallel group to
    all other ranks in the group."""
    ...
    torch.distributed.broadcast(tensor, rank, group)
    return tensor


def broadcast_list_in_dp_group(size, dtype, list_values=None):
    ...
    if mpu.get_pipeline_model_parallel_rank() == 0 \
        ...
        tensor = torch.tensor(list_values, dtype=dtype,
                              device=torch.cuda.current_device())
    return broadcast_tensor_in_dp_group(size, dtype, tensor=tensor)


def broadcast_int_list_in_dp_group(size, int_list=None, rank=0):
    """Broadcast a list of interger values."""

    return broadcast_list_in_dp_group(size, torch.int64, list_values=int_list)