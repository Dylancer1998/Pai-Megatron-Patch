...
from megatron.text_generation.communication import broadcast_int_list_in_dp_group
from megatron.text_generation.communication import broadcast_tensor_in_dp_group
...
def tokenize_prompts(prompts=None,
    ...
    #[dylan's feature] rank should be the smallest rank in DP group, when data parallel size > 1
    ...
    sizes_tensor = broadcast_int_list_in_dp_group(2, int_list=sizes_list)
    
    # Now that we have the sizes, we can boradcast the tokens
    # and length tensors.
    sizes = sizes_tensor.tolist()
    prompts_tokens_cuda_long_tensor = broadcast_tensor_in_dp_group(
        sizes, torch.int64, tensor=prompts_tokens_cuda_long_tensor)
    prompts_length_cuda_long_tensor = broadcast_tensor_in_dp_group(
